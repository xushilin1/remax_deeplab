...
class kMaXDeepLabHead(Mask2FormerHead):
    ...
    def _loss_by_feat_single(self, cls_scores: Tensor, mask_preds: Tensor, sem_preds, pixel_feature: Tensor,
                             batch_gt_instances: List[InstanceData], batch_gt_sem_seg,
    ...
    def forward(self, x: List[Tensor],
        ...
        for i, feat in enumerate(multi_scale_memorys):
            for _ in range(self._num_blocks[i]):
                ...
                sem_seg_pred = None
                if self.remax and self.training:
    ...
    def loss(self, x: Tuple[Tensor], batch_data_samples: SampleList):
        ...
        for gt_instance in batch_gt_instances:
            gt_instance.masks = gt_instance.masks[:, ::4, ::4]
        
        batch_gt_sem_seg = []
        for gt_sem_seg in batch_gt_semantic_segs:
            if gt_sem_seg is not None:
                sem_seg = gt_sem_seg.sem_seg[:,::4,::4]
                sem_seg[sem_seg == 255] = self.num_classes
                batch_gt_sem_seg.append(sem_seg)

        # loss
        losses = self.loss_by_feat(all_cls_scores, all_mask_preds,
                                   all_pixel_feats, all_sem_preds, batch_gt_instances, batch_gt_sem_seg,
                                   batch_img_metas)
    ...
    def loss_by_feat(self, all_cls_scores: Tensor, all_mask_preds: Tensor,
                     all_pixel_feats: Tensor, all_sem_preds,
                     batch_gt_instances: List[InstanceData], batch_gt_sem_seg,
                     batch_img_metas: List[dict]):
        ...
        loss_dict['loss_sem'] = losses_sem[-1]
