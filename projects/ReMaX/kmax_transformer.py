...
class kMaXTransformerLayer(nn.Module):
    ...
    def forward(self, pixel_feature, query_feature):
        ...
        with torch.no_grad():
            ...
            index = clustering_result.max(1, keepdim=True)[1]   # N, HW   
            clustering_result = torch.zeros_like(clustering_result, memory_format=torch.legacy_contiguous_format).scatter_(1, index, 1.0)   # N, L, HW
