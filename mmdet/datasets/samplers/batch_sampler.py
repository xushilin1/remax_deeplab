...
#         for idx in self.sampler:
#             data_info = self.sampler.dataset.get_data_info(idx)
#             # if data_info doesn't contain width and height
#             if 'width' not in data_info or 'height' not in data_info:
#                 aspect_ratio_bucket_id = 0
#             else:
#                 width, height = data_info['width'], data_info['height']
...
class MultiDataAspectRatioBatchSampler(BatchSampler):
    ...
    def __iter__(self) -> Sequence[int]:
        ...
        if world_size > 1:
            ...
            torch_dist.all_reduce(num_batch, op=torch_dist.ReduceOp.MIN, group=group)
