...
class LoadAnnotations(MMCV_LoadAnnotations):
    ...
    def _load_point_coords(self, results: dict) -> None:
        gt_point_coords = []
        for instance in results.get('instances', []):
            gt_point_coords.append(instance['point_coords'][0])
...
class LoadPanopticAnnotations(LoadAnnotations):
    ...
    def __init__(self,
                 ...
                 with_mask: bool = True,
                 with_seg: bool = True,
                 merge_thing_stuff_mask: bool = False,
                 box_type: str = 'hbox',
                 imdecode_backend: str = 'cv2',
        ...
        self.merge_thing_stuff_mask = merge_thing_stuff_mask
    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.merge_thing_stuff_mask:
            img_bytes = get(results['seg_map_path'],
            ...
            h, w = results['ori_shape']
            gt_masks = BitmapMasks(gt_masks, h, w)
            gt_bboxes = gt_masks.get_bboxes(type(results['gt_bboxes']))
            results['gt_masks'] = gt_masks
