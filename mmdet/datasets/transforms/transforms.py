...
class Resize(MMCV_Resize):
    ...
    def _resize_point_coords(self, results: dict) -> None:
        if results.get('gt_point_coords', None) is not None:
            results['gt_point_coords'] = results['gt_point_coords'] * results[
                'scale_factor']
            results['gt_point_coords'][..., 0] = np.clip(
                results['gt_point_coords'][..., 0], 0, results['img_shape'][1])
            results['gt_point_coords'][..., 1] = np.clip(
                results['gt_point_coords'][..., 1], 0, results['img_shape'][0])
    ...
    def transform(self, results: dict) -> dict:
        ...
        results['resize_shape'] = results['img'].shape[:2]
...
class RandomFlip(MMCV_RandomFlip):
    ...
    def _flip(self, results: dict) -> None:
        ...
        if results.get('gt_point_coords', None) is not None:
            ...
            point_coords = results['gt_point_coords'].copy()
            if direction == 'horizontal':
                point_coords[..., 0] = img_shape[1] - point_coords[..., 0]
            elif direction == 'vertical':
                point_coords[..., 1] = img_shape[0] - point_coords[..., 1]
            else:
                point_coords[..., 0] = img_shape[1] - point_coords[..., 0]
                point_coords[..., 1] = img_shape[0] - point_coords[..., 1]
            results['gt_point_coords'] = point_coords
        # flip masks
        if results.get('gt_masks', None) is not None:
...
class RandomCrop(BaseTransform):
    ...
    def _crop_data(self, results: dict, crop_size: Tuple[int, int],
        ...
        if results.get('gt_bboxes', None) is not None:
            ...
            if results.get('gt_point_coords', None) is not None:
                results['gt_point_coords'] = \
                    results['gt_point_coords'][valid_inds]
